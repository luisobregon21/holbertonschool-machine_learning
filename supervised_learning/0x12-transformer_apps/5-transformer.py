#!/usr/bin/env python3
"""Defines a class that inherits from tensorflow.keras.layers.Layer to create\
    transformer network."""
...
def get_angle(pos, i, dm):
    """
    Calculate the angles for the following formulas for positional encoding.
...
class MultiHeadAttention(tf.keras.layers.Layer):
    ...
    def split_heads(self, x, batch):
        """Split the last dimension of tensor into (h, dm) and transposes the\
            result so the shape is (batch, h, seq_len, dm)."""
    ...
    def call(self, Q, K, V, mask):
        """
        Generate the query, key, and value matrices and outputs the scaled dot\
            product attention.
...
class EncoderBlock(tf.keras.layers.Layer):
    ...
    def call(self, x, training, mask=None):
        """
        Call the encoder block and returns the block's output.
...
class DecoderBlock(tf.keras.layers.Layer):
    ...
    def call(self, x, encoder_output, training, look_ahead_mask, padding_mask):
        """
        Call the decoder block and returns the block's output.
...
class Encoder(tf.keras.layers.Layer):
    ...
    def call(self, x, training, mask):
        """
        Call the encoder and returns the encoder's output.
...
class Decoder(tf.keras.layers.Layer):
    ...
    def call(self, x, encoder_output, training, look_ahead_mask, padding_mask):
        """
        Call the decoder and returns the decoder's output.
...
class Transformer(tf.keras.layers.Layer):
    ...
    def call(self, inputs, target, training, encoder_mask, look_ahead_mask,
             decoder_mask):
        """
        Call the transformer network and returns the transformer output.
